...
const adminName = import.meta.env.VITE_APP_ADMIN_NAME || 'Sridhar Kumaresan'
const adminEmailAddress = import.meta.env.VITE_APP_ADMIN_EMAIL_ADDRESS || 'sridharkumaresan4@gmail.com'
...
const sendEmail = async (event) => {
  ...
  if (!publicKey || publicKey === 'YOUR_PUBLIC_KEY' || !serviceId || !templateId) {
    emailSent.value = false
    errorMessage.value = 'EmailJS keys not configured in .env. Please configure .env or contact ' + adminEmailAddress + ' directly.'
    return
  }
  ...
  try {
    await emailjs.send(
      ...
      templateId,
      {
        ...
        to_name: adminName,
        from_name: userEmail.value.split('@')[0] || userEmail.value,
    ...
    errorMessage.value = t('windows.contact.error.unknown') + ' ' + adminEmailAddress + (detailMsg ? ` (${detailMsg})` : '')
  }
}
...
<template>
  <form @submit.prevent="sendEmail" class="relative right-0 h-full flex flex-col h-content-headless-toolbox">
    <!-- Header tools -->
    ...
    <div class="bg-window-white border-window-header-bot w-full h-18 flex items-center flex-col p-2 text-xxs gap-2">
      <label class="w-full flex gap-2 font-trebuchet-pixel">
        ...
        <input
          type="text"
          class="w-full h-5 border border-input-blue p-1.5 text-xs outline-none placeholder:text-black font-trebuchet-pixel"
